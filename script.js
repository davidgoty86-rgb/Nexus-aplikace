    ...
    function defaultState() {
      return {
        ...
        needs: [], questions: [], offerMade: false, benefits: [], utilities: [],
    ...
    function toggleArr(prop, v) {
      ...
      if (prop === 'needs' && !callState.productLocked) inferProduct();
      saveState();
      renderAll();
    }
          ...
          ${blocks.map((b, i) => {
             ...
             return `
              <button class="road-item ${i < currentIndex ? 'done' : i === currentIndex ? 'cur' : ''}" style="${skipped ? 'opacity:.4' : ''}" data-action="go-roadmap-card" data-val="${i}">
                ...
                <span>
                  <div class="rt">${esc(getNavigationName(b))}</div>
    ...
    function renderProductCopilot() {
        ...
        let groupsHtml = Object.values(needGroupsDef).map(g => `
          <div class="need-section ${g.cls}">
            ...
            <div class="need-inner">
              ${g.items.map(n => pill(n, callState.needs.includes(n), 'toggle-arr', 'needs', n)).join('')}
            </div>
          </div>
        `).join('');
      ...
      $('productCard').innerHTML = `
        <div class="prod-title">
          <span>Copilot: ${esc(p.label)}</span>
          <span class="badge" style="background:${p.theme}; color:white; cursor:pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.2);" data-action="toggle-advice" title="Kliknutím zobraz/skryj živou podporu">Živá podpora</span>
        </div>
        
        <div id="adviceBox" style="display: ${copilotAdviceVisible ? 'block' : 'none'}; margin-bottom:8px;">
          <div class="ai-live-border">
            <div class="ai-live-border-inner">
              <h4 style="color:var(--o2);margin-bottom:5px;">Živá AI podpora</h4>
              <div style="font-size:11.5px; line-height:1.45; font-weight:800; color:#1e293b;">
    ...
    function renderNotesScore() {
      ...
      $('notesScoreHost').innerHTML = `
        <div style="background: linear-gradient(135deg, var(--o2d), var(--o2)); color: #fff; border-radius: 14px; padding: 10px 14px; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; box-shadow: 0 4px 12px rgba(0,80,255,0.15);">
          <div>
      ...
      if (R.state === 'warmup') {
        body = `
          <div style="display:flex;align-items:center;gap:14px;padding:8px 4px;">
            <div style="width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,var(--o2d),var(--o2));display:grid;place-items:center;flex-shrink:0;">
        ...
        body = `
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
            <div style="width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,var(--o2d),var(--o2));display:grid;place-items:center;flex-shrink:0;box-shadow:0 4px 12px rgba(0,80,255,.25);">
        ...
        <div style="display:flex;flex-direction:column;height:100%;min-height:0;gap:12px;">
          ...
          <div style="display:flex;align-items:center;gap:12px;flex-shrink:0;background:#fff;padding:10px 16px;border-radius:16px;border:1px solid var(--line);flex-wrap:wrap;">
            ...
            <div style="display:flex;gap:10px;align-items:stretch;flex-shrink:0;">
              <div style="background:linear-gradient(135deg,var(--o2d),var(--o2));color:#fff;border-radius:12px;padding:8px 22px;text-align:center;min-width:90px;">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;flex:1;min-height:0;overflow:hidden;">
            ...
            <!-- LEVÝ: AI kouč + otázky + potřeby -->
            <div style="display:flex;flex-direction:column;gap:10px;min-height:0;overflow:hidden;">
              ${statsExpandedBox ? '' : `
                <div style="flex:2.4;min-height:0;display:flex;flex-direction:column;overflow:hidden;">
                  ${renderCoachPanel(data)}
                </div>
              `}
              ${statsExpandedBox === 'needs' ? '' : `
                <div class="box" data-action="toggle-stats-box" data-val="questions" style="flex:${statsExpandedBox === 'questions' ? '1' : '0.5'};min-height:0;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;transition:flex .25s ease;">
                  <h4 style="flex-shrink:0;color:var(--o2);border-bottom:1px solid var(--line);padding-bottom:6px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span>Nejčastěji používané otázky</span>
                    <span style="font-size:9px;color:var(--mut);font-weight:800;text-transform:uppercase;letter-spacing:.4px;">${statsExpandedBox === 'questions' ? '▼ zavřít' : '▲ rozbalit'}</span>
                  </h4>
                  <div style="display:flex;flex-direction:column;gap:5px;overflow-y:auto;flex:1;min-height:0;padding-right:2px;">
                    ${topQuestions.length ? topQuestions.map(([q, count]) => `
                      <div style="background:#f8fbff;border:1px solid #bfdbfe;border-radius:10px;padding:7px 10px;display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
                        <span style="flex:1;font-size:11.5px;font-weight:800;color:var(--ink);line-height:1.4;">„${esc(q)}"</span>
                        <span style="background:var(--o2);color:#fff;border-radius:999px;padding:2px 8px;font-size:10px;font-weight:950;flex-shrink:0;">${count}×</span>
                      </div>
                    `).join('') : '<div style="color:var(--mut);font-size:11px;padding:4px;">Zatím žádné otázky.</div>'}
                  </div>
                </div>
              `}
              ${statsExpandedBox === 'questions' ? '' : `
                <div class="box" data-action="toggle-stats-box" data-val="needs" style="flex:${statsExpandedBox === 'needs' ? '1' : '0.5'};min-height:0;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;transition:flex .25s ease;">
                  <h4 style="flex-shrink:0;color:#10b981;border-bottom:1px solid var(--line);padding-bottom:6px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;">
                    <span>Co jsem nejčastěji zjistil</span>
                    <span style="font-size:9px;color:var(--mut);font-weight:800;text-transform:uppercase;letter-spacing:.4px;">${statsExpandedBox === 'needs' ? '▼ zavřít' : '▲ rozbalit'}</span>
                  </h4>
                  <div style="display:flex;flex-direction:column;gap:5px;overflow-y:auto;flex:1;min-height:0;padding-right:2px;">
                    ${topNeeds.length ? topNeeds.map(([n, count]) => `
                      <div style="background:#ecfdf5;border:1px solid #6ee7b7;border-radius:10px;padding:7px 10px;display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
                        <span style="flex:1;font-size:11.5px;font-weight:800;color:#065f46;line-height:1.4;">✓ ${esc(n)}</span>
                        <span style="background:#10b981;color:#fff;border-radius:999px;padding:2px 8px;font-size:10px;font-weight:950;flex-shrink:0;">${count}×</span>
                      </div>
                    `).join('') : '<div style="color:var(--mut);font-size:11px;padding:4px;">Zatím žádné potřeby.</div>'}
                  </div>
                </div>
              `}
            </div>
    ...
    function toggleZen() {
      // Cyklus: 0 = vyp, 1 = skrýt boční panely, 2 = vše rozostřené kromě panelu pod myší
      zenLevel = (zenLevel + 1) % 3;
      document.body.classList.toggle('zen-mode', zenLevel === 1);
      document.body.classList.toggle('zen2-mode', zenLevel === 2);
      const btn = $('zenBtn'), badge = $('zenBadge');
      if (btn) {
        btn.classList.remove('zen1', 'zen2');
        if (zenLevel === 1) btn.classList.add('zen1');
        if (zenLevel === 2) btn.classList.add('zen2');
        btn.title = zenLevel === 0
          ? 'Zen režim: klik pro Zen 1 (skrýt boční panely)'
          : zenLevel === 1
            ? 'Zen 1 aktivní: klik pro Zen 2 (rozostřit vše kromě panelu pod myší)'
            : 'Zen 2 aktivní: klik pro vypnutí';
      }
      if (badge) badge.textContent = zenLevel === 0 ? '' : String(zenLevel);
      toast(zenLevel === 0 ? 'Zen režim vypnut' : zenLevel === 1 ? 'Zen 1: skryté boční panely' : 'Zen 2: focus pod kurzorem');
      renderAll();
    }
  ...
  if (!('zoom' in document.body.style) || navigator.userAgent.includes('Firefox')) {
    ...
    function collectVoiceMatches(spokenNormalized) {
      ...
      // idx.needs.forEach(e => tryAdd(e, matches.needs));
    ...
    function applyVoiceMatches(matches) {
      ...
      matches.needs.forEach(v => addToArrayProp('needs', v, 'voice-needs'));
      ...
      if (changedTotal > 0 && added.needs > 0 && !callState.productLocked) inferProduct();
    ...
    function voiceSummaryToast(added) {
      const parts = [];
      if (added.needs) parts.push(`potřeby ${added.needs}`);
      if (added.questions) parts.push(`otázky ${added.questions}`);
      if (added.benefits) parts.push(`výhody ${added.benefits}`);
      if (added.utilities) parts.push(`užitky ${added.utilities}`);
      if (added.nextSteps) parts.push(`dotažení ${added.nextSteps}`);
...
// startAudioMeter();  
    ...
    document.addEventListener('click', e => {
      ...
      if (act === 'toggle-advice') { 
          copilotAdviceVisible = !copilotAdviceVisible; 
          renderProductCopilot(); 
      }
    ...
    function loadTeamsConfig() {
      try {
        ...
        if (!raw) return { targets: [], defaultTargetId: null, authorName: 'David Gottfried' };
        ...
        if (raw && typeof raw === 'object' && ('webhookUrl' in raw) && !Array.isArray(raw.targets)) {
          if (raw.webhookUrl) {
            ...
            return { targets: [t], defaultTargetId: t.id, authorName: 'David Gottfried' };
          }
          return { targets: [], defaultTargetId: null, authorName: 'David Gottfried' };
        }
        return {
      ...
      } catch(e) { return { targets: [], defaultTargetId: null, authorName: 'David Gottfried' }; }
    }
    ...
    function addTeamsTarget() {
      teamsConfig.targets.push({ id: _uid(), name: 'Nový cíl', webhookUrl: '', enabled: false });
    ...
    // Send an Office 365 MessageCard to a specific webhook URL.
    function _teamsPostCard(url, card) {
      return fetch(url, {
        method: 'POST',
    ...
    function _buildSnapCard(snap) {
      const statusEmoji = { 'Prodáno': '✅', 'Zpětný kontakt': '🔄', 'Neprodáno': '❌' };
      ...
      const needsList = (snap.needs||[]).slice(0,3).join(', ') || '—';
      const scoreColor = (snap.score||0) >= 80 ? '00b050' : (snap.score||0) >= 60 ? 'f59e0b' : 'dc2626';
      return {
        '@type': 'MessageCard',
        '@context': 'https://schema.org/extensions',
        'summary': 'Nexus: O2 Sales Copilot – ' + status,
        'themeColor': scoreColor,
        'title': '📞 **Nexus:** O2 Sales Copilot – ' + status,
        'sections': [{
          'facts': [
            { 'name': 'Výsledek:', 'value': em + ' ' + status },
            { 'name': 'Skóre:', 'value': (snap.score||0) + '%' },
            { 'name': 'Produkt:', 'value': snap.product || '—' },
            { 'name': 'Délka:', 'value': durStr },
            { 'name': 'Potřeby:', 'value': needsList }
          ],
          'text': getAuthorName() + ' · Nexus: O2 Sales Copilot · ' + (snap.time||'') + ' · ' + (snap.date||'')
        }]
      };
    }
    ...
    function _buildTextCard(title, text) {
      return {
        ...
        'sections': [{
          'text': text,
          'facts': [{ 'name': 'Odesláno od:', 'value': getAuthorName() }]
        }]
      };
    }

    function sendTeamsNotification(snap) {
      // Auto-notify: send to ALL enabled targets (default + others). No target? → skip silently.
      const enabled = teamsConfig.targets.filter(t => t.enabled && t.webhookUrl);
      if (!enabled.length) return;
      const card = _buildSnapCard(snap);
      const promises = enabled.map(t => _teamsPostCard(t.webhookUrl, card).catch(() => null));
      Promise.all(promises).then(() => {
        toast(`📢 Teams: notifikace odeslána (${enabled.length}× cíl)`);
      });
    }
    ...
    function _buildStatsCard(data, period, authorName) {
      ...
      // Součty per-produkt pro každý z 3 per-produkt grafů
      const perProductTotals = (histField, productField) => {
        ...
        return STATS_PROD_DEFS.map(p => `${p.label}: ${totals[p.key] || 0}`).join(' · ');
      };

      const questions = perProductTotals('questions', 'questions');
      const benefits  = perProductTotals('benefits', 'copilotBenefits');
      const utilities = perProductTotals('utilities', 'copilotUtilities');

      return {
        '@type': 'MessageCard',
        '@context': 'https://schema.org/extensions',
        'summary': `Statistiky ${periodLabel} — ${authorName}`,
        'themeColor': '0050ff',
        'title': `📊 **Nexus:** O2 Sales Copilot — Statistiky (${periodLabel})`,
        'sections': [
          {
            'activityTitle': `Odesláno od: **${authorName}**`,
            'activitySubtitle': `${new Date().toLocaleDateString('cs-CZ')} · ${new Date().toLocaleTimeString('cs-CZ',{hour:'2-digit',minute:'2-digit'})}`,
            'facts': [
              { 'name': '📞 Hovory celkem:', 'value': String(callsTotal) },
              { 'name': '🖱️ Kliky celkem:', 'value': String(clicksTotal) }
            ]
          },
          {
            'activityTitle': '**1) Hovory ' + (calls.isHourly ? 'po hodinách' : 'po dnech') + '**',
            'text': 'Top: ' + topN(calls)
          },
          {
            'activityTitle': '**2) Kliky ' + (clicks.isHourly ? 'po hodinách' : 'po dnech') + '**',
            'text': 'Top: ' + topN(clicks)
          },
          {
            'activityTitle': '**3) Profilační otázky dle produktu**',
            'text': questions
          },
          {
            'activityTitle': '**4) Výhody dle produktu**',
            'text': benefits
          },
          {
            'activityTitle': '**5) Užitky dle produktu**',
            'text': utilities
          },
          {
            'text': '_Vizuální grafy (obrázek) najdeš v následující zprávě od autora._'
          }
        ]
      };
    }
